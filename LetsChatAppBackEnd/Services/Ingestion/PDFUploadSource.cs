...
public class PDFUploadSource(string uploadDirectory) : IIngestionSource
{
    ...
    public string SourceId => $"{nameof(PDFUploadSource)}:{uploadDirectory}";
    ...
    public Task<IEnumerable<IngestedDocument>> GetNewOrModifiedDocumentsAsync(IReadOnlyList<IngestedDocument> existingDocuments)
    {
        ...
        var sourceFiles = Directory.GetFiles(uploadDirectory, "*.pdf");
    ...
    public Task<IEnumerable<IngestedDocument>> GetDeletedDocumentsAsync(IReadOnlyList<IngestedDocument> existingDocuments)
    {
        ...
        var currentFiles = Directory.GetFiles(uploadDirectory, "*.pdf");
    ...
    {
        using var pdf = PdfDocument.Open(Path.Combine(uploadDirectory, document.DocumentId));
...
#pragma warning restore SKEXP0050 // Type is for evaluation purposes only
    ...
    public string GetUploadDirectory() => uploadDirectory;
}
